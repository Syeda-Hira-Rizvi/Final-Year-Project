...
function CheckEligibility() {
  ...
  const [isQuizButtonClicked, setIsQuizButtonClicked] = useState({
    blood: false,
    stemCell: false,
  }); // To track if the quiz button is clicked for each type
  ...
  const handleYes = () => {
    if (step < questions.length) {
      ...
      setIsEligible(true); // User answered "Yes" to all
      setStep(questions.length + 1);
    }
  };

  const handleNo = () => {
    setIsEligible(false); // User is ineligible
    setStep(questions.length + 1); // Move to result screen
  };
        ...
        <MiddleContentSection>
          ...
          <MiddleDescription>
            <ul>
              ...
              <li>Not have donated in the last 3 months. This waiting period allows the body to recover from any previous donation, ensuring that the donor is in optimal health for the next donation process.</li>

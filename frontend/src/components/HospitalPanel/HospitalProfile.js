...
function HospitalProfile() {
  ...
  const fetchHospitalInfo = async () => {
    try {
      ...
      // setMessage({ text: "Error fetching hospital info", success: false });
  ...
  const handleSave = async () => {
    try {
      ...
      // setMessage({ text: "Error updating hospital info", success: false });
  ...
  //   } catch (error) {
  //     setMessage({ text: error.response?.data?.message || "Error changing password", success: false });
  //   }
  // };
  const handlePasswordUpdate = async () => {

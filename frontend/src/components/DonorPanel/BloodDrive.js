  ...
  useEffect(() => {
    const fetchBloodDrives = async () => {
      try {
        ...
        // const upcomingDrives = response.data.bloodDrives.filter((drive) => {
        //   const endDate = new Date(drive.endDate);
        //   const isDateValid = endDate >= today;

        //   const currentVolunteers = drive.volunteers?.length || 0;
        //   const isCapacityAvailable = currentVolunteers < drive.volunteerCapacity;

        //   return isDateValid && isCapacityAvailable;
        // });

        // setBloodDrives(upcomingDrives);

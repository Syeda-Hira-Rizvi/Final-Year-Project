...
//   res.status(200).json({
//       success: true,
//       message: "Password updated successfully for Hospital",
//   });
// });
...
// Get a specific donor by ID for a specific hospital
exports.getHospitalDonorById = catchAsyncErrors(async (req, res, next) => {
  const { hospitalId, donorId } = req.params;

  if (!hospitalId || !donorId) {
    return next(new ErrorHandler('Hospital ID and Donor ID are required', 400));
  }

  // Fetch appointments for the specific hospital and donor
  ...
  if (!appointments || appointments.length === 0) {
    return next(new ErrorHandler('Donor not found for this hospital', 404));
  }

  // Since donor details are populated, extract the donor from the first matched appointment
  const donor = appointments[0].donor;
...
// Get a specific volunteer by ID for a specific hospital
// Get a specific volunteer by ID for a specific hospital
exports.getVolunteerById = catchAsyncErrors(async (req, res, next) => {
  const { hospitalId, volunteerId } = req.params;

  if (!hospitalId || !volunteerId) {
    return next(new ErrorHandler('Hospital ID and Volunteer ID are required', 400));
  }

  // Fetch appointments for the specific hospital and volunteer
  const appointments = await Appointment.find({
    hospital: hospitalId,
    donor: volunteerId
  })
  ...
  if (!appointments || appointments.length === 0) {
    return next(new ErrorHandler('Volunteer not found for this hospital', 404));
  }

  // Extract the volunteer from the first matched appointment and ensure they are marked as a volunteer
  const volunteer = appointments[0].donor;

  if (!volunteer || !volunteer.isVolunteer) {
    return next(new ErrorHandler('This donor is not a volunteer for this hospital', 404));
  }
...
const generateYearlySlots = (operationalHours, slotDuration, operationalDays, year) => {
  ...
  for (let month = 0; month < 12; month++) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      // // Agar date past hai, to directly N/A mark kar do
      // if (dateStr < today) {
      //   yearlySlots.push({ date: dateStr, slots: [{ time: "N/A", isAvailable: false }] });
      //   continue;
      // }
...
//     const slots = generateSlots(openingTime, closingTime, slotDuration, false);
//     weekSlots.push({ date: date.toISOString().split("T")[0], slots });
//   }
...
// // Add Slots Controller (Now supports updating existing slots)
// exports.addSlots = catchAsyncErrors(async (req, res, next) => {
//   const { date, yearWide, year } = req.body;

//   // Fetch hospital based on logged-in user's ID
//   const hospital = await Hospital.findOne({ user: req.user.id });
...
//   if (!yearWide) {
//     // Single date slot generation
//     const parsedDate = new Date(date);

//     if (isNaN(parsedDate)) {
//       return next(new ErrorHandler("Invalid date provided", 400));
//     }

//     if (parsedDate < new Date()) {
//       return next(new ErrorHandler("Cannot add slots for past dates", 400));
//     }
...
//     const slots = generateSlots(openingTime, closingTime, slotDuration, false);
//     const existingIndex = hospital.availability.findIndex((item) => item.date === date);

//     if (existingIndex !== -1) {
//       // Update existing slots
//       hospital.availability[existingIndex].slots = slots;
//     } else {
//       // Add new slots
//       hospital.availability.push({ date, slots });
//     }
//   } else {
//     // Yearly slot generation
//     const selectedYear = year || new Date().getFullYear();
//     const yearlySlots = generateYearlySlots(operationalHours, slotDuration, operationalDays, selectedYear);

//     // Remove existing slots for the selected year to prevent duplication
//     hospital.availability = hospital.availability.filter(
...
exports.getAllSlots = catchAsyncErrors(async (req, res, next) => {
  // Fetch hospital based on logged-in user's ID
  const hospital = await Hospital.findOne({ user: req.user.id });
  ...
  res.status(200).json({
    success: true,
    availability: hospital.availability, // Sare available slots bhej raha hai
  });
});
...
// Delete Slot Controller (Supports Single Date and Yearly Deletion)
exports.deleteSlots = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!date && !year) {
    return next(new ErrorHandler("Please provide a date or a year to delete slots", 400));
  }

  if (date) {
    // Delete slots for a specific date
    const index = hospital.availability.findIndex((item) => item.date === date);

    if (index === -1) {
      return next(new ErrorHandler("Slot not found for the given date", 404));
    }

    hospital.availability.splice(index, 1);
  }
  else if (year) {
    // Delete all slots for a given year
    const initialLength = hospital.availability.length;
    hospital.availability = hospital.availability.filter(item => !item.date.startsWith(`${year}-`));

    if (hospital.availability.length === initialLength) {
      return next(new ErrorHandler(`No slots found for the year ${year}`, 404));
    }
  }
  ...
  res.status(200).json({
    success: true,
    message: date ? `Slots for ${date} deleted successfully` : `All slots for year ${year} deleted successfully`,
  });
});
...
// Blood Donation Appointment Confirmation Controller
exports.confirmBloodDonationAppointment = catchAsyncErrors(async (req, res, next) => {
  const hospitalId = req.user.id; // Get hospital ID from authenticated user

  // Validate if the authenticated user is a hospital
  const hospital = await User.findById(hospitalId);
  if (!hospital || hospital.role !== 'hospital') {
    return next(new ErrorHandler('Only hospitals can confirm appointments', 403));
  }

  const { donorId, appointmentDate, appointmentTime, donationType } = req.body;

  // Validate required details
  if (!donorId || !appointmentDate || !appointmentTime || !donationType) {
    return next(new ErrorHandler('Missing required details for confirming appointment', 400));
  }

  // Find the donor to ensure they exist and are eligible
  const donor = await User.findById(donorId);
  if (!donor || donor.role !== 'donor') {
    return next(new ErrorHandler('Donor not found or invalid', 404));
  }

  // Find the existing blood donation appointment
  const appointment = await Appointment.findOne({
    donor: donorId,
    appointmentDate,
    appointmentTime,
    status: 'Pending', // Only confirm pending appointments
    //donationType: 'Blood Donation', // Ensure it's a blood donation appointment
    donationType,
  });

  if (!appointment) {
    // return next(new ErrorHandler('No pending blood donation appointment found to confirm', 404));
    return next(new ErrorHandler(`No pending  ${donationType} found to confirm`, 404));
  }
  ...
  // // Now, create the blood donation record
  // const donation = await Donation.create({
  //   donor: donorId,
  //   hospital: hospitalId,
  //   hospitalName: hospital.user.name, // Save the hospital name
  //   // donationType: 'Blood',
  //   donationType,
  //   status: 'Pending',
  //   appointment: appointment._id, // Link to the confirmed appointment
  // });


  // Determine donation phase for Stem Cell Donation
  ...
  // Return confirmation details and donation record
  res.status(200).json({
    success: true,
    // message: 'Blood donation appointment confirmed and donation created',
    message: `${donationType} appointment confirmed and donation created`,
    donation,
    appointment,
  });
});
...
// Confirm HLA test Appointment Controller (Hospital confirms the appointment)(not required now)
exports.confirmHLATestAppointment = catchAsyncErrors(async (req, res, next) => {
  const hospitalId = req.user.id; // Get hospital ID from the authenticated user
  console.log('Authenticated User ID:', hospitalId); // Debug the user ID

  // Find the hospital and populate the associated user to check the role
  // const hospital = await Hospital.findById(hospitalId).populate('user'); // Populate the 'user' field
  const hospital = await User.findById(hospitalId); // Populate the 'user' field
  ...
  if (!donorId || !appointmentDate || !appointmentTime) {
    return next(
      new ErrorHandler('Missing required details for confirming appointment', 400)
    );
  }
  ...
  if (!appointment) {
    return next(new ErrorHandler('No pending appointment found to confirm', 404));
  }
  ...
  // Now, create the donation record for HLA Testing phase
  const donation = await Donation.create({
    donor: donorId,
    hospital: hospitalId,
    hospitalName: hospital.user.name, // Save the hospital name
  ...
  res.status(200).json({
    success: true,
    message: 'Appointment confirmed successfully by the hospital',
    appointment,
    donation
  });
});
...
// Confirm Stem Cell Donation Appointment Controller (Hospital confirms the appointment)(not required now)
exports.confirmStemCellAppointment = catchAsyncErrors(async (req, res, next) => {
  const hospitalId = req.user.id; // Get hospital ID from the authenticated user
  ...
  // Find the existing pending stem cell donation appointment
  const appointment = await Appointment.findOne({
    donor: donorId,
  ...
  if (!appointment) {
    return next(new ErrorHandler('No pending stem cell donation appointment found to confirm', 404));
  }

  // Update the appointment date, time, and status
  appointment.appointmentDate = appointmentDate;
  appointment.appointmentTime = appointmentTime;
  appointment.status = 'Confirmed'; // Update status to 'Confirmed'
  await appointment.save();

  // Create the stem cell donation record
  const donation = await Donation.create({
    donor: donorId,
    hospital: hospitalId,
    hospitalName: hospital.user.name, // Save the hospital name
    donationType: 'Stem Cell', // Specify donation type
    status: 'Pending', // Initial status
    appointment: appointment._id, // Link the donation to the appointment
  });

  // Return confirmation details and donation record
  res.status(200).json({
    success: true,
    message: 'Stem cell donation appointment confirmed and donation created successfully by the hospital',
    donation,
    appointment,
  });
});

...
// Get donor donations
exports.getDonorDonations = catchAsyncErrors(async (req, res, next) => {
  const { donorId } = req.params; // Donor ID from route params

  if (!donorId) {
    return next(new ErrorHandler('Donor ID is required', 400));
  }

  // Fetch donations for the specific donor
  const donations = await Donation.find({ donor: donorId }).populate(
    'hospital',
    'name location' // Populate hospital details if needed
  );

  if (!donations || donations.length === 0) {
    return next(new ErrorHandler('No donations found for this donor', 404));
  }
...
// //WORKING CANCEL APPOINTMENT CONTROLLER
...
//   // Fetch donor's confirmed appointments
//   //const appointments = await Appointment.find({ donor: donorId, status: 'Confirmed' })
//   const appointments = await Appointment.find({ donor: donorId })
//     // .populate({
//     //   path: 'hospital',
//     //   select: 'name address', // Adjusting to use fields from User model
//     // })
//     // .select('-donor');
//     .populate({
//       path: 'hospital',
...
exports.getAvailableSlots = catchAsyncErrors(async (req, res, next) => {
  ...
  // Log for requested date
  console.log("Request Parameters:", req.params);

  // Find availability for the given date
  const availability = hospital.availability.find((slot) => slot.date === new Date(date).toISOString().split('T')[0]);

  if (!availability) {
    return next(new ErrorHandler('No slots available for the selected date', 404));
  }
...
exports.applyAsVolunteer = catchAsyncErrors(async (req, res, next) => {
  ...
  //   (volunteer) => new Date(volunteer.availability).toDateString() === new Date(availability).toDateString()
  // );
  const volunteersOnDate = bloodDrive.registeredVolunteers.filter(
  ...
  // // Add the user to the drive's registered volunteers  (NOT REQUIRED HERE USE IT IN ACCEPT VOULNTEER APPLICATION CONTROLLER)
  // bloodDrive.registeredVolunteers.push({
  //   donor: userId,
  //   hospital: bloodDrive.hospital,
  //   //availability,
  //   date: availability,
  // });
